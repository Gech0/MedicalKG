    ...
    def spider_main(self):
        for page in range(1, 11000):
            try:
                ...
                data = {}
                data['url'] = basic_url
                data['basic_info'] = self.basicinfo_spider(basic_url)       # 简介
                data['cause_info'] = self.common_spider(cause_url)          # 病因
                data['prevent_info'] = self.common_spider(prevent_url)      # 预防
                data['symptom_info'] = self.symptom_spider(symptom_url)     # 症状
                data['inspect_info'] = self.inspect_spider(inspect_url)     # 检查
                data['treat_info'] = self.treat_spider(treat_url)           # 治疗
                data['food_info'] = self.food_spider(food_url)              # 饮食保健
                data['drug_info'] = self.drug_spider(drug_url)              # 好评药品
